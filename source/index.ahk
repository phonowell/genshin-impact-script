...
global __ci_genshin__ := Func("genshin_144")
if (A_IsAdmin != true) {
...
class TickerX extends EmitterShellX {
  ...
  update := Func("genshin_142").Bind(this)
}
...
class ConfigX {
  ...
  read := Func("genshin_141").Bind(this)
}
...
class ConsoleX {
  ...
  __New() {
    ...
    ticker.on.Call("change", Func("genshin_140").Bind(this))
    $.on.Call("alt + f9", Func("genshin_139").Bind(this))
  }
  clean := Func("genshin_138").Bind(this)
  hide := Func("genshin_137").Bind(this)
  log := Func("genshin_136").Bind(this)
  pickColor := Func("genshin_135").Bind(this)
  render := Func("genshin_134").Bind(this)
...
class ClientX extends KeyBindingX {
  ...
  __New() {
    base.__New()
    this.setSize.Call()
    ticker.on.Call("change", Func("genshin_132").Bind(this))
    this.on.Call("enter", Func("genshin_131").Bind(this))
    $.on.Call("alt + f4", Func("genshin_130").Bind(this))
    $.on.Call("ctrl + f5", Func("genshin_129").Bind(this))
    $.on.Call("alt + enter", Func("genshin_128").Bind(this))
  }
...
class PlayerX extends KeyBindingX {
  ...
  __New() {
    ...
    this.bindEvent.Call("view", "m-button")
...
class MovementX extends KeyBindingX {
  ...
  checkMove := Func("genshin_63").Bind(this)
}
...
class TacticX {
  ...
  get := Func("genshin_37").Bind(this)
  jump := Func("genshin_36").Bind(this)
  ongoing := Func("genshin_35").Bind(this)
  onMoving := Func("genshin_34").Bind(this)
  reset := Func("genshin_33").Bind(this)
  sprint := Func("genshin_32").Bind(this)
  start := Func("genshin_31").Bind(this)
  stop := Func("genshin_30").Bind(this)
  toggle := Func("genshin_29").Bind(this)
  useE := Func("genshin_28").Bind(this)
  validate := Func("genshin_27").Bind(this)
  wait := Func("genshin_26").Bind(this)
}
global tactic := new TacticX()
ts.jump := 0
player.on.Call("jump:start", Func("genshin_25")).on.Call("unhold:start", Func("genshin_24"))
timer.pick := ""
global pick := Func("genshin_23")
global skip := Func("genshin_22")
global startPick := Func("genshin_21")
global stopPick := Func("genshin_20")
player.on.Call("pick:start", startPick).on.Call("pick:end", stopPick)
state.isSprinting := false
state.isSprintSwimming := false
timer.sprint := ""
ts.sprint := 0
global checkSwimming := Func("genshin_19")
global sprint := Func("genshin_18")
global startSprint := Func("genshin_17")
global stopSprint := Func("genshin_16")
global swim := Func("genshin_15")
player.on.Call("sprint:start", startSprint).on.Call("sprint:end", stopSprint).on.Call("move:start", Func("genshin_14")).on.Call("move:end", Func("genshin_13"))
state.toggleDelay := 150
global getToggleDelay := Func("genshin_12")
global startToggle := Func("genshin_11")
global stopToggle := Func("genshin_9")
player.on.Call("toggle:start", startToggle).on.Call("toggle:end", stopToggle).on.Call("toggle-q:end", player.toggleQ)
player.on.Call("use-e:start", Func("genshin_6")).on.Call("use-e:end", Func("genshin_5")).on.Call("use-q:start", player.useQ)
for __index_for__, key in [1, 2, 3, 4] {
  $.on.Call("alt + " . (key) . "", Func("genshin_4").Bind(key))
}
...
genshin_9() {
  ...
  if (typeApr == 2) {
    $.setTimeout.Call(Func("genshin_8"), state.toggleDelay)
    return
  }
...
genshin_11(key) {
  ...
  $.setTimeout.Call(Func("genshin_10"), state.toggleDelay)
}
genshin_12() {
...
genshin_24() {
  $.press.Call("x")
  if !(Config.data.betterJump && !menu.checkVisibility.Call()) {
    return
  }
  $.sleep.Call(200)
  player.jump.Call()
...
genshin_28(this, isHolding, callback) {
...
genshin_35(this, cbA, cbB, isNot := false) {
...
genshin_58(this, listTactic, g := 0, i := 0) {
  ...
  if !(callback) {
    callback := Func("genshin_38").Bind(item, next, this)
  }
  callback.Call()
}
...
genshin_62(this, isCharged, callback) {
  ...
  if (isCharged) {
    ...
    __object__ := player
    name := __object__["name"]
    switch Character.data[__ci_genshin__.Call(name)].weapon {
      case "bow": {
        delay := 1500
        if (name == "ganyu") {
          delay := 1800
...
genshin_140(this, tick) {
  if !($.mod.Call(tick, 500)) {
    this.update.Call()
  }
}
genshin_141(this, key, defaultValue := "") {
