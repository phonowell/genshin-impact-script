; Generated by Coffee-AHK/0.0.20
global __ci_genshin__ := Func("genshin_52")
if (A_IsAdmin != true) {
...
class ClientX {
  ...
  getSize := Func("genshin_50").Bind(this)
  isActive := Func("genshin_49").Bind(this)
  reset := Func("genshin_48").Bind(this)
  resetKey := Func("genshin_47").Bind(this)
  resetTimer := Func("genshin_46").Bind(this)
  suspend := Func("genshin_45").Bind(this)
  setPriority := Func("genshin_44").Bind(this)
  watch := Func("genshin_43").Bind(this)
}
class ConfigX {
  data := {character: [""]}
  source := "config.ini"
  __New() {
    ...
    for __index_for__, key in ["autoESkill", "easySkillTimer", "fastPaimonMenu", "fastPick", "improvedElementalVision", "improvedJump", "improvedSprint"] {
...
timer.countDown := ""
global countDown := Func("genshin_41")
class PaimonX {
  color := [0xFFFFFF, 0xECE5D8, 0x3B4255]
  key := ["f1", "f2", "f3", "f4", "f5", "b", "c", "j", "m"]
  state := {current: "", isVisible: false}
  ts := {check: 0}
  bindEvent := Func("genshin_40").Bind(this)
  checkVisibility := Func("genshin_36").Bind(this)
  findColor := Func("genshin_35").Bind(this)
...
genshin_39(this, key) {
  ...
  if (this.state.isVisible) {
    ...
    timer.closePaimon := setTimeout.Call(Func("genshin_38").Bind(this, key), 1000)
    return
  }
  this.state.current := key
  this.state.isVisible := true
...
genshin_40(this) {
  for __index_for__, key in this.key {
    $.on.Call(key, Func("genshin_39").Bind(this, key))
    $.on.Call("esc", Func("genshin_37").Bind(this))
  }
}
genshin_41(time) {
  if !(config.data.easySkillTimer) {
...
genshin_42(this, key, defaultValue := 1) {
...
genshin_44(this, level) {
  Process, Priority, % config.data.process, % level
}
genshin_45(this, isSuspend) {
  if (isSuspend) {
