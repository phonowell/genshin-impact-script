...
global __ci_genshin__ := Func("genshin_51")
if (A_IsAdmin != true) {
...
class ClientX {
  ...
  width := 0
  check := Func("genshin_50").Bind(this)
  getSize := Func("genshin_49").Bind(this)
  isActive := Func("genshin_48").Bind(this)
  reset := Func("genshin_47").Bind(this)
  resetKey := Func("genshin_46").Bind(this)
  resetTimer := Func("genshin_45").Bind(this)
  suspend := Func("genshin_44").Bind(this)
  setPriority := Func("genshin_43").Bind(this)
  watch := Func("genshin_42").Bind(this)
}
class ConfigX {
  data := {character: [""]}
  source := "config.ini"
  ...
  read := Func("genshin_41").Bind(this)
}
timer.countDown := ""
global countDown := Func("genshin_40")
class PaimonX {
  color := [0xFFFFFF, 0xECE5D8, 0x3B4255]
  key := ["f1", "f2", "f3", "f4", "f5", "b", "c", "j", "m"]
  state := {current: "", isVisible: false}
  ts := {check: 0}
  bindEvent := Func("genshin_39").Bind(this)
  checkVisibility := Func("genshin_35").Bind(this)
  findColor := Func("genshin_34").Bind(this)
  resetKey := Func("genshin_33").Bind(this)
}
state.isDashing := false
timer.dash := ""
ts.dash := 0
global dash := Func("genshin_32")
global startDash := Func("genshin_30")
global stopDash := Func("genshin_29")
state.isJumping := false
timer.jump := ""
global jump := Func("genshin_28")
global jumpTwice := Func("genshin_26")
...
global startPick := Func("genshin_18")
global stopPick := Func("genshin_17")
...
genshin_15(key) {
  ...
  timer.toggle := $.delay.Call(getToggleDelay.Call(), Func("genshin_14").Bind(key))
}
genshin_16() {
  limit := [200, 600]
  delay := limit[1]
...
genshin_25(callback) {
  ...
  timer.jump := $.delay.Call(100, Func("genshin_24").Bind(callback))
}
genshin_26(callback) {
  $.press.Call("space")
  clearTimeout.Call(timer.jump)
  timer.jump := $.delay.Call(200, Func("genshin_25").Bind(callback))
}
genshin_27() {
  state.isJumping := false
}
genshin_28() {
  ...
  jumpTwice.Call(Func("genshin_27"))
}
genshin_29() {
  ts.dash := $.now.Call()
  if !(config.data.improvedSprint) {
...
genshin_32() {
  ...
  timer.dash := $.delay.Call(1300, Func("genshin_31"))
}
genshin_33(this) {
...
genshin_38(this, key) {
  ...
  if (this.state.isVisible) {
    ...
    timer.closePaimon := setTimeout.Call(Func("genshin_37").Bind(this, key), 1000)
    return
  }
  this.state.current := key
  this.state.isVisible := true
...
genshin_39(this) {
  for __index_for__, key in this.key {
    $.on.Call(key, Func("genshin_38").Bind(this, key))
    $.on.Call("esc", Func("genshin_36").Bind(this))
  }
}
genshin_40(time) {
  if !(config.data.easySkillTimer) {
...
genshin_41(this, key, defaultValue := 1) {
...
genshin_43(this, level) {
  Process, Priority, % config.data.process, % level
}
genshin_44(this, isSuspend) {
  if (isSuspend) {
