; Generated by Coffee-AHK/0.0.31
global __ci_genshin__ := Func("genshin_86")
if (A_IsAdmin != true) {
...
class EmitterX {
  bus := []
  emit := Func("genshin_85").Bind(this)
  off := Func("genshin_84").Bind(this)
  on := Func("genshin_83").Bind(this)
}
class TickerX extends EmitterX {
  ...
  update := Func("genshin_82").Bind(this)
}
...
class ConfigX {
  ...
  read := Func("genshin_81").Bind(this)
}
...
class ClientX extends EmitterX {
  ...
  checkActive := Func("genshin_79").Bind(this)
  reset := Func("genshin_78").Bind(this)
  resetTimer := Func("genshin_77").Bind(this)
  setSize := Func("genshin_76").Bind(this)
  suspend := Func("genshin_75").Bind(this)
  setPriority := Func("genshin_74").Bind(this)
  vh := Func("genshin_73").Bind(this)
  vw := Func("genshin_72").Bind(this)
}
global client := new ClientX()
ticker.on.Call("change", Func("genshin_71"))
class ConsoleX {
  lifetime := 5000
  list := []
  tsClean := 0
  check := Func("genshin_70").Bind(this)
  clean := Func("genshin_69").Bind(this)
  hide := Func("genshin_68").Bind(this)
  log := Func("genshin_67").Bind(this)
  render := Func("genshin_66").Bind(this)
}
global console := new ConsoleX()
ticker.on.Call("change", Func("genshin_65"))
class HudX {
  lifetime := 5000
  listTimer := {}
  getColor := Func("genshin_64").Bind(this)
  getPosition := Func("genshin_63").Bind(this)
  hide := Func("genshin_62").Bind(this)
  render := Func("genshin_61").Bind(this)
...
class CharacterX {
  ...
  __New() {
    this.index := $.split.Call((this.read.Call("index/index")), ",")
    for __index_for__, name in this.index {
      this[__ci_genshin__.Call(name)] := {cd: $.split.Call((this.read.Call("" . (name) . "/cd", "0,0")), ","), color: this.read.Call("" . (name) . "/color", 0), typeApr: this.read.Call("" . (name) . "/type-apr", 1), typeAtk: this.read.Call("" . (name) . "/type-atk", 1), typeE: this.read.Call("" . (name) . "/type-e", 0)}
    }
  }
  read := Func("genshin_58").Bind(this)
}
...
class MemberX {
  ...
  map := {}
  name := ""
  checkCharacterByPosition := Func("genshin_57").Bind(this)
  getRange := Func("genshin_56").Bind(this)
  scan := Func("genshin_55").Bind(this)
  toggle := Func("genshin_54").Bind(this)
}
global member := new MemberX()
class SkillTimerX {
  listCountDown := {}
  listRecord := {}
  check := Func("genshin_53").Bind(this)
  record := Func("genshin_52").Bind(this)
  recordEnd := Func("genshin_51").Bind(this)
  recordStart := Func("genshin_50").Bind(this)
  reset := Func("genshin_49").Bind(this)
}
global skillTimer := new SkillTimerX()
if (Config.data.easySkillTimer) {
  ticker.on.Call("change", Func("genshin_48"))
}
class PlayerX extends EmitterX {
  isMoving := false
  startMove := Func("genshin_47").Bind(this)
  stopMove := Func("genshin_46").Bind(this)
  toggleQ := Func("genshin_45").Bind(this)
  useE := Func("genshin_44").Bind(this)
  useQ := Func("genshin_42").Bind(this)
}
...
class MovementX extends EmitterX {
  ...
  __New() {
    ...
    for __index_for__, key in ["w", "a", "s", "d"] {
      $.on.Call(key, Func("genshin_31").Bind(key, this))
      $.on.Call("" . (key) . ":up", Func("genshin_30").Bind(key, this))
    }
  }
  check := Func("genshin_29").Bind(this)
  checkMove := Func("genshin_28").Bind(this)
  resetKey := Func("genshin_27").Bind(this)
}
global movement := new MovementX()
player.on.Call("move-start", Func("genshin_26")).on.Call("move-end", Func("genshin_25"))
timer.pick := ""
...
timer.sprint := ""
ts.sprint := 0
global checkSwimming := Func("genshin_21")
global sprint := Func("genshin_20")
global startSprint := Func("genshin_18")
global stopSprint := Func("genshin_17")
player.on.Call("sprint-start", startSprint).on.Call("sprint-end", stopSprint)
if (Config.data.betterSprint) {
  player.on.Call("move-end", Func("genshin_16"))
}
...
genshin_20() {
  ...
  clearTimeout.Call(timer.sprint)
  timer.sprint := $.delay.Call(1300, Func("genshin_19"))
}
genshin_21() {
...
genshin_35() {
  $.delay.Call(200, Func("genshin_34"))
}
genshin_36() {
  $.press.Call("m")
}
...
genshin_41(this, name, key, key2 := "") {
  $.on.Call("" . (key) . "", Func("genshin_40").Bind(key, key2, name, this))
  $.on.Call("" . (key) . ":up", Func("genshin_39").Bind(key, key2, name, this))
  return this
}
genshin_42(this) {
  $.press.Call("q")
...
genshin_44(this, isHolding := false) {
  if !(isHolding) {
    $.press.Call("e")
    skillTimer.record.Call("start")
    skillTimer.record.Call("end")
  ...
  $.press.Call("e:down")
  skillTimer.record.Call("start")
  $.delay.Call(1000, Func("genshin_43"))
  return this
}
genshin_45(this, key) {
  $.press.Call("alt + " . (key) . "")
...
genshin_58(this, key, defaultValue := "") {
...
genshin_61(this, n, msg) {
  clearTimeout.Call(this.listTimer[__ci_genshin__.Call(n)])
  this.listTimer[__ci_genshin__.Call(n)] := setTimeout.Call(Func("genshin_60").Bind(n, this), this.lifetime)
...
genshin_74(this, level) {
  Process, Priority, % Config.data.process, % level
}
genshin_75(this, isSuspend) {
  if (isSuspend) {
    if (this.isSuspend) {
      return
    }
...
genshin_81(this, key, defaultValue := 1) {
  __array__ := $.split.Call(key, "/")
  __section__ := __array__[1]
  __key__ := __array__[2]
  IniRead, __result__, % this.source, % __section__, % __key__, % defaultValue
