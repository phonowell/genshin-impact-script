...
global __ci_genshin__ := Func("genshin_49")
if (A_IsAdmin != true) {
...
class ClientX {
  ...
  getSize := Func("genshin_47").Bind(this)
...
class PaimonX {
  ...
  ts := {check: 0}
  bindEvent := Func("genshin_37").Bind(this)
  checkVisibility := Func("genshin_33").Bind(this)
  findColor := Func("genshin_32").Bind(this)
...
global startDash := Func("genshin_28")
global stopDash := Func("genshin_27")
state.isJumping := false
timer.jump := ""
global jump := Func("genshin_26")
...
genshin_23(callback) {
  ...
  timer.jump := $.delay.Call(100, Func("genshin_22").Bind(callback))
}
genshin_24(callback) {
  $.press.Call("space")
  clearTimeout.Call(timer.jump)
  timer.jump := $.delay.Call(200, Func("genshin_23").Bind(callback))
}
genshin_25() {
  state.isJumping := false
}
genshin_26() {
  ...
  jumpTwice.Call(Func("genshin_25"))
}
genshin_27() {
  ts.dash := $.now.Call()
  if !(config.data.improvedSprint) {
...
genshin_30() {
  ...
  timer.dash := $.delay.Call(1300, Func("genshin_29"))
}
genshin_31(this) {
...
genshin_36(this, key) {
  ...
  if (this.state.isVisible) {
    ...
    timer.closePaimon := setTimeout.Call(Func("genshin_35").Bind(this, key), 1000)
    return
  }
  this.state.current := key
  this.state.isVisible := true
...
genshin_37(this) {
  for __index_for__, key in this.key {
    $.on.Call(key, Func("genshin_36").Bind(this, key))
    $.on.Call("esc", Func("genshin_34").Bind(this))
  }
}
genshin_38(time) {
  if !(config.data.easySkillTimer) {
