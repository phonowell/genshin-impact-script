...
global __ci_genshin__ := Func("genshin_85")
if (A_IsAdmin != true) {
...
class EmitterX {
  bus := []
  emit := Func("genshin_84").Bind(this)
  off := Func("genshin_83").Bind(this)
  on := Func("genshin_82").Bind(this)
}
class TickerX extends EmitterX {
  ...
  update := Func("genshin_81").Bind(this)
}
...
class ConfigX {
  ...
  read := Func("genshin_80").Bind(this)
}
...
class ClientX extends EmitterX {
  ...
  checkActive := Func("genshin_78").Bind(this)
  reset := Func("genshin_77").Bind(this)
  resetTimer := Func("genshin_76").Bind(this)
  setSize := Func("genshin_75").Bind(this)
  suspend := Func("genshin_74").Bind(this)
  setPriority := Func("genshin_73").Bind(this)
  vh := Func("genshin_72").Bind(this)
  vw := Func("genshin_71").Bind(this)
}
global client := new ClientX()
ticker.on.Call("change", Func("genshin_70"))
class ConsoleX {
  lifetime := 5000
  list := []
  tsClean := 0
  check := Func("genshin_69").Bind(this)
  clean := Func("genshin_68").Bind(this)
  hide := Func("genshin_67").Bind(this)
  log := Func("genshin_66").Bind(this)
  render := Func("genshin_65").Bind(this)
}
global console := new ConsoleX()
ticker.on.Call("change", Func("genshin_64"))
class HudX {
  lifetime := 5000
  listTimer := {}
  getColor := Func("genshin_63").Bind(this)
  getPosition := Func("genshin_62").Bind(this)
  hide := Func("genshin_61").Bind(this)
  render := Func("genshin_60").Bind(this)
  reset := Func("genshin_58").Bind(this)
}
global hud := new HudX()
...
class CharacterX {
  ...
  read := Func("genshin_57").Bind(this)
}
...
class MemberX {
  ...
  map := {}
  name := ""
  checkCharacterByPosition := Func("genshin_56").Bind(this)
  getRange := Func("genshin_55").Bind(this)
  scan := Func("genshin_54").Bind(this)
  toggle := Func("genshin_53").Bind(this)
}
global member := new MemberX()
class SkillTimerX {
  listCountDown := {}
  listRecord := {}
  check := Func("genshin_52").Bind(this)
  record := Func("genshin_51").Bind(this)
  recordEnd := Func("genshin_50").Bind(this)
  recordStart := Func("genshin_49").Bind(this)
  reset := Func("genshin_48").Bind(this)
}
global skillTimer := new SkillTimerX()
if (Config.data.easySkillTimer) {
  ticker.on.Call("change", Func("genshin_47"))
}
class PlayerX extends EmitterX {
  isMoving := false
  toggleQ := Func("genshin_46").Bind(this)
}
...
class KeyBindingX extends EmitterX {
  ...
  bindEvent := Func("genshin_45").Bind(this)
  resetKey := Func("genshin_42").Bind(this)
}
...
class MovementX extends EmitterX {
  ...
  check := Func("genshin_36").Bind(this)
  checkMove := Func("genshin_35").Bind(this)
  preventDefault := Func("genshin_34").Bind(this)
  resetKey := Func("genshin_31").Bind(this)
  startMove := Func("genshin_30").Bind(this)
  stopMove := Func("genshin_29").Bind(this)
}
global movement := new MovementX()
ticker.on.Call("change", movement.check)
player.on.Call("move-start", Func("genshin_28")).on.Call("move-end", Func("genshin_27"))
timer.pick := ""
global pick := Func("genshin_26")
global stopPick := Func("genshin_24")
...
timer.sprint := ""
ts.sprint := 0
global checkSwimming := Func("genshin_23")
global sprint := Func("genshin_22")
global startSprint := Func("genshin_20")
global stopSprint := Func("genshin_19")
player.on.Call("sprint-start", startSprint).on.Call("sprint-end", stopSprint)
if (Config.data.betterSprint) {
  $.on.Call("w:up", Func("genshin_18"))
}
...
genshin_13(key) {
  ...
  if (typeApr == 3) {
    $.press.Call("e:down")
    skillTimer.record.Call("start")
    $.delay.Call(1000, Func("genshin_12"))
    return
  }
  if (typeApr == 4) {
    $.press.Call("q")
...
genshin_15(key) {
  ...
  timer.toggleDown := $.delay.Call(state.toggleDelay, Func("genshin_14"))
}
genshin_16() {
...
genshin_22() {
  ...
  clearTimeout.Call(timer.sprint)
  timer.sprint := $.delay.Call(1300, Func("genshin_21"))
}
genshin_23() {
...
genshin_26() {
  clearTimeout.Call(timer.pick)
  timer.pick := $.delay.Call(100, Func("genshin_25"))
}
genshin_27() {
  if !(player.isMoving) {
    return
...
genshin_34(this) {
  for __index_for__, key in ["w", "a", "s", "d"] {
    $.on.Call(key, Func("genshin_33"))
    $.on.Call("" . (key) . ":up", Func("genshin_32"))
  }
  return this
}
genshin_35(this) {
...
genshin_45(this, name, key, key2 := "") {
  $.on.Call("" . (key) . "", Func("genshin_44").Bind(key, key2, name, this))
  $.on.Call("" . (key) . ":up", Func("genshin_43").Bind(key, key2, name, this))
  return this
}
genshin_46(this, key) {
  $.press.Call("alt + " . (key) . "")
...
genshin_57(this, key, defaultValue := "") {
...
genshin_60(this, n, msg) {
  clearTimeout.Call(this.listTimer[__ci_genshin__.Call(n)])
  this.listTimer[__ci_genshin__.Call(n)] := setTimeout.Call(Func("genshin_59").Bind(n, this), this.lifetime)
...
genshin_73(this, level) {
  Process, Priority, % Config.data.process, % level
}
genshin_74(this, isSuspend) {
  if (isSuspend) {
    if (this.isSuspend) {
      return
    }
...
genshin_80(this, key, defaultValue := 1) {
  __array__ := $.split.Call(key, "/")
  __section__ := __array__[1]
  __key__ := __array__[2]
  IniRead, __result__, % this.source, % __section__, % __key__, % defaultValue
