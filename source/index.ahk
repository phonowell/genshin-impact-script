...
global __ci_genshin__ := Func("genshin_65")
if (A_IsAdmin != true) {
...
class ClientX {
  ...
  close := Func("genshin_63").Bind(this)
  getSize := Func("genshin_62").Bind(this)
  isActive := Func("genshin_61").Bind(this)
  reset := Func("genshin_60").Bind(this)
  resetKey := Func("genshin_59").Bind(this)
  resetTimer := Func("genshin_58").Bind(this)
  suspend := Func("genshin_57").Bind(this)
  setPriority := Func("genshin_56").Bind(this)
  watch := Func("genshin_55").Bind(this)
}
class ConfigX {
  data := {character: [""]}
  source := "config.ini"
  ...
  read := Func("genshin_54").Bind(this)
}
timer.countDown := ""
global countDown := Func("genshin_53")
class PaimonX {
  color := [0xFFFFFF, 0xECE5D8, 0x3B4255]
  key := ["f1", "f2", "f3", "f4", "f5", "b", "c", "j", "m"]
  state := {current: "", isVisible: false}
  ts := {check: 0}
  bindEvent := Func("genshin_52").Bind(this)
  checkVisibility := Func("genshin_48").Bind(this)
  findColor := Func("genshin_47").Bind(this)
  resetKey := Func("genshin_46").Bind(this)
}
global combo := Func("genshin_45").Call()
global klee := Func("genshin_42").Call()
global tactic := {combo: combo, klee: klee}
timer.tactic := ""
state.isAttacking := false
global setTacticDelay := Func("genshin_35")
global startAttack := Func("genshin_33")
global stopAttack := Func("genshin_32")
state.isDashing := false
timer.dash := ""
ts.dash := 0
global dash := Func("genshin_31")
...
global isMoving := Func("genshin_22")
global pauseMove := Func("genshin_21")
global resumeMove := Func("genshin_20")
...
genshin_19() {
  ...
  timer.pick := $.delay.Call(100, Func("genshin_18"))
}
genshin_20() {
...
genshin_31() {
  ...
  timer.dash := $.delay.Call(1300, Func("genshin_30"))
}
genshin_32() {
  state.isAttacking := false
...
genshin_35(delay, callback) {
  clearTimeout.Call(timer.tactic)
  timer.tactic := $.delay.Call(delay, Func("genshin_34").Bind(callback))
}
genshin_36(module) {
  $.press.Call("w")
...
genshin_37(module) {
  $.click.Call("left:up")
  setTacticDelay.Call(850, Func("genshin_36").Bind(module))
}
genshin_38(module) {
  $.click.Call("left:down")
  setTacticDelay.Call(250, Func("genshin_37").Bind(module))
}
genshin_39(module) {
  $.press.Call("w")
  setTacticDelay.Call(50, module.attack)
...
genshin_40(module) {
  $.click.Call("left")
  setTacticDelay.Call(550, Func("genshin_39").Bind(module))
}
genshin_41(module) {
...
genshin_42() {
  ...
  module.attackB := Func("genshin_38").Bind(module)
  return module
}
genshin_43(module) {
  $.click.Call("left")
...
genshin_45() {
  module := {}
  module.attack := Func("genshin_44").Bind(module)
  module.attackA := Func("genshin_43").Bind(module)
  return module
}
genshin_46(this) {
...
genshin_51(this, key) {
  ...
  if (this.state.isVisible) {
    ...
    timer.closePaimon := setTimeout.Call(Func("genshin_50").Bind(this, key), 1000)
    return
  }
  this.state.current := key
  this.state.isVisible := true
...
genshin_52(this) {
  for __index_for__, key in this.key {
    $.on.Call(key, Func("genshin_51").Bind(this, key))
    $.on.Call("esc", Func("genshin_49").Bind(this))
  }
}
genshin_53(time) {
  if !(config.data.easySkillTimer) {
...
genshin_54(this, key, defaultValue := 1) {
...
genshin_56(this, level) {
  Process, Priority, % this.name, % level
}
genshin_57(this, isSuspend) {
  if (isSuspend) {
