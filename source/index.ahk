...
global __ci_genshin__ := Func("genshin_99")
if (A_IsAdmin != true) {
...
class EmitterX {
  bus := []
  emit := Func("genshin_98").Bind(this)
  off := Func("genshin_97").Bind(this)
  on := Func("genshin_96").Bind(this)
}
class TickerX extends EmitterX {
  ...
  update := Func("genshin_95").Bind(this)
}
...
class ConfigX {
  ...
  read := Func("genshin_94").Bind(this)
}
...
class ClientX extends EmitterX {
  ...
  check := Func("genshin_93").Bind(this)
  checkActive := Func("genshin_92").Bind(this)
  point := Func("genshin_91").Bind(this)
  reset := Func("genshin_90").Bind(this)
  resetTimer := Func("genshin_89").Bind(this)
  setSize := Func("genshin_88").Bind(this)
  suspend := Func("genshin_87").Bind(this)
  setPriority := Func("genshin_86").Bind(this)
  vh := Func("genshin_85").Bind(this)
  vw := Func("genshin_84").Bind(this)
}
global client := new ClientX()
ticker.on.Call("change", Func("genshin_83"))
class ConsoleX {
  lifetime := 5000
  list := []
  tsClean := 0
  ...
  clean := Func("genshin_81").Bind(this)
  hide := Func("genshin_80").Bind(this)
  log := Func("genshin_79").Bind(this)
  render := Func("genshin_78").Bind(this)
}
...
class HudX {
  lifetime := 5000
  listTimer := {}
  getColor := Func("genshin_76").Bind(this)
  getPosition := Func("genshin_75").Bind(this)
...
class MemberX {
  ...
  map := {}
  name := ""
  checkCharacterByPosition := Func("genshin_70").Bind(this)
  getRange := Func("genshin_69").Bind(this)
  scan := Func("genshin_68").Bind(this)
  toggle := Func("genshin_67").Bind(this)
}
...
class SkillTimerX {
  ...
  recordEnd := Func("genshin_63").Bind(this)
  recordStart := Func("genshin_62").Bind(this)
  reset := Func("genshin_61").Bind(this)
}
global skillTimer := new SkillTimerX()
if (Config.data.easySkillTimer) {
  ticker.on.Call("change", Func("genshin_60"))
}
...
class PlayerX extends KeyBindingX {
  ...
  startMove := Func("genshin_53").Bind(this)
  stopMove := Func("genshin_52").Bind(this)
  toggleQ := Func("genshin_51").Bind(this)
  useE := Func("genshin_50").Bind(this)
  useQ := Func("genshin_48").Bind(this)
}
...
class MovementX extends KeyBindingX {
  ...
  __New() {
    ...
    for __index_for__, key in ["w", "a", "s", "d"] {
      $.on.Call(key, Func("genshin_47").Bind(key, this))
      $.on.Call("" . (key) . ":up", Func("genshin_46").Bind(key, this))
    }
  }
  check := Func("genshin_45").Bind(this)
  checkMove := Func("genshin_44").Bind(this)
}
...
if (Config.data.betterElementalVision) {
  player.on.Call("find:end", Func("genshin_41"))
}
global jumpTwice := Func("genshin_39")
if (Config.data.betterJump) {
  player.on.Call("jump:end", jumpTwice).on.Call("unhold:end", Func("genshin_37"))
}
timer.pick := ""
global pick := Func("genshin_35")
global stopPick := Func("genshin_33")
...
timer.sprint := ""
ts.sprint := 0
global checkSwimming := Func("genshin_32")
global sprint := Func("genshin_31")
global startSprint := Func("genshin_29")
global stopSprint := Func("genshin_28")
player.on.Call("sprint:start", startSprint).on.Call("sprint:end", stopSprint)
if (Config.data.betterSprint) {
  player.on.Call("move:end", Func("genshin_27"))
}
state.toggleDelay := 200
global getToggleDelay := Func("genshin_26")
global startToggle := Func("genshin_25")
global stopToggle := Func("genshin_23")
player.on.Call("toggle:start", startToggle).on.Call("toggle:end", stopToggle)
for __index_for__, key in [1, 2, 3, 4] {
  $.on.Call("alt + " . (key) . "", Func("genshin_20").Bind(key))
}
if (Config.data.easySkillTimer) {
  player.on.Call("use-e:start", Func("genshin_19")).on.Call("use-e:end", Func("genshin_18"))
}
state.isViewing := false
timer.view := ""
global toggleView := Func("genshin_17")
global view := Func("genshin_16")
if (Config.data.betterElementalVision) {
...
class TacticX {
  ...
  checkStatus := Func("genshin_14").Bind(this)
...
genshin_14(this) {
  now := $.now.Call()
  if !(now - ts.tacticCheckStatus > 1000) {
...
genshin_16() {
  ...
  setTimeout.Call(Func("genshin_15"), 2500)
}
genshin_17() {
...
genshin_25(key) {
  ...
  $.delay.Call(state.toggleDelay, Func("genshin_24"))
}
genshin_26() {
...
genshin_31() {
  ...
  clearTimeout.Call(timer.sprint)
  timer.sprint := $.delay.Call(1300, Func("genshin_30"))
}
genshin_32() {
...
genshin_35() {
  clearTimeout.Call(timer.pick)
  timer.pick := $.delay.Call(100, Func("genshin_34"))
}
genshin_36() {
  $.press.Call("space")
}
genshin_37() {
  $.delay.Call(50, Func("genshin_36"))
}
genshin_38() {
  if !(player.isMoving) {
    return
...
genshin_41() {
  $.delay.Call(200, Func("genshin_40"))
}
genshin_42() {
  if !(player.isMoving) {
    return
...
genshin_50(this, isHolding := false) {
  if !(isHolding) {
    $.press.Call("e")
    skillTimer.record.Call("start")
    skillTimer.record.Call("end")
  ...
  $.press.Call("e:down")
  skillTimer.record.Call("start")
  $.delay.Call(1000, Func("genshin_49"))
  return this
}
genshin_51(this, key) {
  $.press.Call("alt + " . (key) . "")
...
genshin_73(this, n, msg) {
  clearTimeout.Call(this.listTimer[__ci_genshin__.Call(n)])
  this.listTimer[__ci_genshin__.Call(n)] := setTimeout.Call(Func("genshin_72").Bind(n, this), this.lifetime)
...
genshin_86(this, level) {
  Process, Priority, % Config.data.process, % level
}
genshin_87(this, isSuspend) {
  if (isSuspend) {
    if (this.isSuspend) {
      return
    }
...
genshin_94(this, key, defaultValue := "") {
